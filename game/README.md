...
- `io.axoniq.labs.game.querymodel.rooms.messages`  
...
2. Define a queue called 'player-events'
...
2. We need to tell Spring to use this source when messages arrive on the `player-events` queue:
   ...
   c. Annotate the `onMessage` method with `@RabbitListener(queues = "player-events")`

The `@RabbitListener` tells Spring that we want this method invoked when a message is available on the 
`player-events` queue. Axon will then take it from there, and notify any Processors that are told to read from 
this source.

And that's what we need to do next:
1. Assign the `RoomParticipantsProjection` class to the `player` processing group. (You've done this before, 
remember?)
